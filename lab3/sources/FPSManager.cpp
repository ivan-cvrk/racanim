...
void FPSManager::init(GLFWwindow *Window, double theTargetFps,
                      bool theVerboseSetting) {
  window = Window;
...
FPSManager::FPSManager(GLFWwindow *window, int theTargetFps,
                       float theReportInterval, std::string theWindowTitle) {
  init(window, theTargetFps,
       true); // If you specify a window title it's safe to say you want the FPS
              // to update there ;)
...
void FPSManager::setTargetFps(int theFpsLimit) {
  ...
  // ...then set it and calculate the target duration of each frame at this
  // framerate
...
void FPSManager::setReportInterval(float theReportInterval) {
  // Ensure the time interval between FPS checks is sane (low cap = 0.1s,
  // high-cap = 10.0s) Negative numbers are invalid, 10 fps checks per second at
  // most, 1 every 10 secs at least.
...
double FPSManager::enforceFPS(bool shouldSleep) {
  ...
  // Calculate how long it's been since the frameStartTime was set (at the end
  // of this method)
  ...
  if (reportInterval != 0.0f) {
    ...
    if ((frameEndTime - lastReportTime) > reportInterval) {
      ...
      // Calculate the FPS as the number of frames divided by the interval in
      // seconds
      currentFps = (double)frameCount / reportInterval;

      // Reset the frame counter to 1 (and not zero - which would make our FPS
      // values off)
      frameCount = 1;
    ...
    } else // FPS calculation time interval hasn't elapsed yet? Simply increment
           // the FPS frame counter
    {
      ++frameCount;
    }
  ...
  // Reset the frame start time to be now - this means we only need put a single
  // call into the main loop
  frameStartTime = glfwGetTime();

  // Pass back our total frame duration (including any sleep and the time it
  // took to run this function) to be used as our deltaTime value
