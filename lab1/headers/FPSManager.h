...
private:
  ...
  double frameDuration; // How many milliseconds between the last frame and this
                        // frame
  ...
  int totalFrameCount; // How many frames have been drawn since the program
                       // start

  double targetFrameDuration; // How many milliseconds each frame should take to
                              // hit a target FPS value (i.e. 60fps = 1.0 / 60 =
                              // 0.016ms)
  double sleepDuration; // How long to sleep if we're exceeding the target frame
                        // rate duration
  ...
  bool verbose; // Whether or not to output FPS details to the console or update
                // the window

  // Limit the minimum and maximum target FPS value to relatively sane values
  static const double MIN_TARGET_FPS;
  static const double
      MAX_TARGET_FPS; // If you set this above the refresh of your monitor and
                      // enable VSync it'll break! Be aware!

  // Private method to set relatively sane defaults. Called by constructors
  // before overwriting with more specific values as required.
...
public:
  ...
  // Note: No FPS reporting by default, although you can turn it on or off later
  // with the setVerbose(true/false) method
  FPSManager(GLFWwindow *window, int theTargetFps);

  // Two parameter constructor which sets a desired framerate and a reporting
  // interval in seconds
  FPSManager(GLFWwindow *window, int theTargetFps, double theReportInterval);

  // Three parameter constructor which sets a desired framerate, how often to
  // report, and the window title to append the FPS to
  ...
  // Setter for the report interval (how often the FPS is reported) - santises
  // input.
  void setReportInterval(float theReportInterval);

  // Method to force our application to stick to a given frame rate and return
  // how long it took to process a frame
